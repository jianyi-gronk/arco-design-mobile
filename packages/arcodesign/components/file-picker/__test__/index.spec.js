...
describe('FilePicker', () => {
    ...
    it('FilePicker renders correctly', () => {
        const { container } = render(<FilePicker files={[]} />);
        expect(container.querySelectorAll(`.${prefix}-add`).length).toBe(1);
    });
    it('FilePicker renders correctly', () => {
        const { container } = render(<FilePicker files={[{ url: IMG_URL }, { url: IMG_URL }]} />);
        expect(container.querySelectorAll(`.${imagePrefix}`).length).toBe(2);
    });
    it('FilePicker renders correctly', () => {
        const { container } = render(
            <FilePicker files={[{ url: IMG_URL }, { url: IMG_URL }]} limit={1} />,
        );
    ...
    it('upload correctly', async () => {
        function App() {
            ...
            return <FilePicker files={files} onChange={setImages} />;
        }
    ...
    it('custom upload', async () => {
        function App() {
            ...
            const [files, setImages] = React.useState([]);
            return <FilePicker files={files} onChange={setImages} upload={mockUpload} />;
        }
        const { container } = render(<App />);
    ...
    it('click image correctly', () => {
        ...
        const { container } = render(
            <FilePicker files={[{ url: IMG_URL }]} onClick={handleClick} />,
        );
    ...
    it('press image correctly', () => {
        ...
        const { container } = render(
            <FilePicker files={[{ url: IMG_URL }]} onLongPress={handleLongPress} />,
        );
        act(() => {
    ...
    it('selectAdapter', async () => {
        ...
        function App({ onMaxSizeExceed, onLimitExceed }) {
            const [files, setImages] = React.useState([{ url: IMG_URL }, { url: IMG_URL }]);
            const selectAdapter = () => {
